...
<div class="row justify-content-center">
    ...
    >		                <div class="app-card app-card-settings shadow-sm p-4">
                                ...
                                @if ($errors->any())
                                    ...
                                    <form class="settings-form" method="POST" action="/salvar_plataforma/{{$relatorio->id}}">
                                        @csrf
                                        @method('PUT')
                                ...
                                <div class="mb-3"  >
                                            ...
                                            <input type="text" class="form-control" id="descricao" name="descricao" value="{{$relatorio->descricao}}" required>
              ...
              <option value="{{ $categorias->id }}" {{ $relatorio->id_categoria == $categorias->id ? 'selected' : '' }}>{{ $categorias->nome }}</option>
              @endforeach
                                           ...
                                           <select type="select" class="form-select"  name="plataformas" id="plataformas" required>
                                            ...
                                            <option value="{{ $plataforma->id }}" {{ $relatorio->id_plataforma == $plataforma->id ? 'selected' : '' }}>{{ $plataforma->nome }}</option>
      ...
      <div class="mb-3">
                                           ...
                                           <select type="select" class="form-select"  name="mes" id="mes" required>
                                            ...
                                            @foreach($mes as $mes)
                                            <option value="{{ $mes->id }}" {{ $relatorio->id_mes== $mes->id ? 'selected' : '' }}>{{ $mes->nome }}</option>
      ...
      <div class="mb-3"  >
                                             ...
                                             <input type="number" class="form-control" id="ano" name="ano" value="{{$relatorio->ano}}" required>

                                 </div>

                                 <button type="submit" class="btn app-btn-primary">Salvar</button>
                                        <a href="{{ url('/lista_plataforma') }}"class="btn btn-secondary">Cancelar</a>
                                    </form>
                                </div><!--//app-card-body-->
